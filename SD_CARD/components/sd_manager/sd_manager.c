...
esp_err_t sd_manager_init(sd_manager_handle_t *sd_obj, const sd_manager_config_t *config, sdmmc_host_t *host,sdmmc_card_t *card)
{
    ...
    esp_err_t err = esp_vfs_fat_sdspi_mount(BASE_PATH, host, &slot_config, &mount_config, &card);
...
esp_err_t sd_manager_write_file(sd_manager_handle_t *sd_obj, const char *file_name, const char *record_mode, const char *data)
{
    ...
    fprintf(f, "%s\n", data);
    /*
    // 2. FORÇA a escrita física no cartão (ESSENCIAL)
    if (fflush(f) != 0) {   
        ESP_LOGE(TAG, "Erro ao realizar flush (Cartão removido?)");
        fclose(f);
        xSemaphoreGive(sd_obj->mutex);
        return ESP_FAIL;
    }*/
    ...
    int ret = ferror(f);
    bool erro_de_escrita = false;

    if (ret != 0) {
        int erro_cod = errno; 
        ESP_LOGE(TAG, "Erro de I/O detectado na escrita! Código errno: %d", erro_cod);
        //ESP_LOGE(TAG, "Erro fatal: falha na leitura do arquivo (I/O error).");
    ...
    if (erro_de_escrita) {
        ESP_LOGE(TAG, "Fim da escrita do arquivo com erros.");
        return ESP_FAIL;
    }
    else
    {
        ESP_LOGI(TAG, "Fim da escrita do arquivo com sucesso.");
        return ESP_OK;
    }
}
...
esp_err_t sd_manager_read_file(sd_manager_handle_t *sd_obj, const char *file_name)
{
    ...
    if (ret != 0) {
        int erro_cod = errno; 
        ESP_LOGE(TAG, "Erro de I/O detectado na leitura! Código errno: %d", erro_cod);
        //ESP_LOGE(TAG, "Erro fatal: falha na leitura do arquivo (I/O error).");
        erro_de_leitura = true;
    }

    fclose(f); //Lbera buffers e fecha o arquivo
    //ESP_LOGI(TAG, "Fim da leitura do arquivo.");

    xSemaphoreGive(sd_obj->mutex);

    if (erro_de_leitura) {
        ESP_LOGI(TAG, "Fim da leitura do arquivo com erros.");
        return ESP_FAIL;
    }
    else
    {
        ESP_LOGI(TAG, "Fim da leitura do arquivo com sucesso.");
        return ESP_OK;
    }
}
...
esp_err_t sd_manager_exists(sd_manager_handle_t *sd_obj, const char *file_name)
{
    ...
    if (ret == 0) {
        ESP_LOGI(TAG, "Arquivo %s existe.", file_name);
        return ESP_OK;
    } else {
        ESP_LOGI(TAG, "Arquivo %s não existe.", file_name);
