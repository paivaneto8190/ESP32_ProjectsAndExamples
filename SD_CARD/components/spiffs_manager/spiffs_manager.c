...
esp_err_t spiffs_manager_format(spiffs_manager_handle_t *spiffs_obj, const char *file_path)
{
    ...
    if (f == NULL) {
        ESP_LOGI(TAG, "Falha ao abrir arquivo para escrita");
        xSemaphoreGive(spiffs_obj->mutex);
        return ESP_FAIL;
    }

    //fprintf(f, ""); // Grava o cabeçalho, por exemplo
    fclose(f);
    return ESP_OK;
}
...
esp_err_t spiffs_manager_read(spiffs_manager_handle_t *spiffs_obj, const char *file_path)
{
    if (!spiffs_obj)
    {
        ESP_LOGE(TAG, "Parâmetros inválidos ou vazios");
...
esp_err_t spiffs_manager_delete_file(spiffs_manager_handle_t *spiffs_obj, const char *file_path)
{
    ...
    if (mutex_taken == pdFALSE)
    {
        ESP_LOGE(TAG, "Falha ao tomar mutex para deletar arquivo");
        return ESP_ERR_TIMEOUT;
    } 

    int res = unlink(file_path);
    if (res != 0) {
        ESP_LOGE(TAG, "Falha ao deletar arquivo");
        xSemaphoreGive(spiffs_obj->mutex);
        return ESP_FAIL; // Falha ao deletar o arquivo
    }
...
esp_err_t spiffs_manager_exists(spiffs_manager_handle_t *spiffs_obj, const char *file_path)
{
    ...
    if (mutex_taken == pdFALSE)
    {
        ESP_LOGE(TAG, "Falha ao tomar mutex para verificar existência de arquivo");
        return ESP_ERR_TIMEOUT;
    } 

    FILE* f = fopen(file_path, "r");
    if (f == NULL) {
        ESP_LOGI(TAG, "Arquivo não existe: %s", file_path);
        xSemaphoreGive(spiffs_obj->mutex);
        return ESP_FAIL; // O arquivo não existe
    }

    fclose(f);
    ESP_LOGI(TAG, "Arquivo existe: %s", file_path);
    xSemaphoreGive(spiffs_obj->mutex);
    return ESP_OK; // O arquivo existe
}
